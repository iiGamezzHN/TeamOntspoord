...
def route2_hill(network, max_length, tot_crit_tracks, stations, n_best, L_crit_tracks):
    while True:
        start = s2(network, L_crit_tracks, stations)
        route = rc.Route(start, [start], 0, 0, L_crit_tracks, 0)
        # Find set of n_best routes
        routes_list = best.route2_object_n_best(network, max_length, route,
                                                tot_crit_tracks, n_best)
        ...
        for item in routes_list:
            ...
            while len(L_crit_tracks) != 0:
                start = s2(network, L_crit_tracks, stations)
                route = rc.Route(start, [start], 0, 0, L_crit_tracks, 0)
                next = best.route2_object_n_best(network, max_length, route,
                                                        tot_crit_tracks, 1)
